...
def curses_print(string, line=line_number):
	if use_curses == True:
...
def rc_go_to_alt(goal_alt):
	...
	Can hold with about .02 meter precision but gets worse as time progresses
	"""
	error = goal_alt - get_alt()
	screen.addstr(5,0, "Throttle RC Level: " + str(v.channel_readback['3']))
	screen.addstr(6,0, "Error: " + str(error))
	screen.addstr(7,0, "Altitude:" + str(get_alt()))
	rc_throttle(1370+error*7.5/1000)
	screen.refresh()
...
def rc_go_to_heading(goal_heading):
	...
	Imput is in radians
	"""
	error = goal_heading - get_yaw()
	screen.addstr(9,0, "Yaw RC Level: " + str(v.channel_readback['4']))
	screen.addstr(10,0, "Error: " + str(error))
	screen.addstr(11,0, "Heading: " + str(get_yaw()))
	rc_yaw(1500+error*180)
	screen.refresh()
...
def rc_go_to_xy(goal_x, goal_y):
	...
	x_error = goal_x - current_x
	y_error = goal_y - current_y
	screen.addstr(13,0, "Pitch RC Level: " + str(v.channel_readback['2']))
	screen.addstr(14,0, "Roll RC Level: " + str(v.channel_readback['1']))
	screen.addstr(15,0, "X Error: " + str(x_error))
	screen.addstr(16,0, "Y Error: " + str(y_error))
	screen.addstr(17,0, "X, Y Position: " + str(x_current) + ", " + str(y_current))
	rc_pitch(1500)
	rc_roll(1500)
	screen.refresh()
...
#	rc_go_to_alt(20000)
#	rc_go_to_heading(2)
	curses_print("Time: " + str((time.clock()-timer)*10))
	curses_print("X position: " + str(get_position()[0]))
	curses_print("Y position: " + str(get_position()[1]))
	curses_print("Z position: " + str(get_position()[2]))
	time.sleep(.1)
